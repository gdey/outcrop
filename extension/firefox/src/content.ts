// content.ts — injected on demand via browser.scripting.executeScript.
//
...
//
// All messaging goes via browser.runtime.sendMessage / onMessage to the
...
(function main(): void {
  ...
  function sendCancel(): void {
    ...
    browser.runtime.sendMessage(m).catch(() => {});
    teardown();
  }
  ...
  function populatePreview(idoc: Document, msg: CroppedMessage): void {
    ...
    const save = (): void => {
      ...
      browser.runtime.sendMessage(out).catch((e) => console.error(e));
      teardown();
    };
