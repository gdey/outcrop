...
func (s *Store) CreateVault(ctx context.Context, v Vault) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO vaults (key, display_name, path, clipping_path, attachment_path, created_at)
		VALUES (?, ?, ?, ?, ?, ?)
	`, v.Key, v.DisplayName, v.Path, v.ClippingPath, v.AttachmentPath, v.CreatedAt.Unix())
	if err != nil {
...
func (s *Store) GetVault(ctx context.Context, key string) (Vault, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		SELECT key, display_name, path, clipping_path, attachment_path, created_at
		FROM vaults WHERE key = ?
	`, key).Scan(&v.Key, &v.DisplayName, &v.Path, &v.ClippingPath, &v.AttachmentPath, &createdAt)
	switch {
...
func (s *Store) ListVaults(ctx context.Context) ([]Vault, error) {
	...
	for rows.Next() {
		var v Vault
		var createdAt int64
		if err := rows.Scan(&v.Key, &v.DisplayName, &v.Path, &v.ClippingPath, &v.AttachmentPath, &createdAt); err != nil {
